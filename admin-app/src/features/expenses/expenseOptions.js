...
const EXPENSE_OPTIONS_KEY = 'guardian-group.expense-options';
const readSavedOptions = () => {
  try { return JSON.parse(window.localStorage.getItem(EXPENSE_OPTIONS_KEY) || 'null') || {}; } catch { return {}; }
};
const validOptions = (items, fallback) => Array.isArray(items) && items.length && items.every((item) => item?.value && item?.label) ? items : fallback;
const savedOptions = readSavedOptions();
export const EXPENSE_CATEGORIES = validOptions(savedOptions.categories, DEFAULT_EXPENSE_CATEGORIES);
export const EXPENSE_TYPES = validOptions(savedOptions.types, DEFAULT_EXPENSE_TYPES);

export function saveExpenseOptions({ categories, types }) {
  const next = {
    categories: validOptions(categories, DEFAULT_EXPENSE_CATEGORIES),
    types: validOptions(types, DEFAULT_EXPENSE_TYPES),
  };
  window.localStorage.setItem(EXPENSE_OPTIONS_KEY, JSON.stringify(next));
}

export function resetExpenseOptions() {
  window.localStorage.removeItem(EXPENSE_OPTIONS_KEY);
}
...
export function categoryForExpenseType(type) { return EXPENSE_TYPES.find((option) => option.value === type)?.category || TYPE_CATEGORY[type] || 'other_business_expense'; }
