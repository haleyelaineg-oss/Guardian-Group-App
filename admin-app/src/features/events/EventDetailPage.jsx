...
export default function EventDetailPage() {
  ...
  const reload = useCallback(() => {
    if (!creating) fetchEvent(id).then(setEvent).catch(error => alert(error.message));
  }, [creating, id]);
  useEffect(() => {
    reload();
  }, [reload]);
  ...
  const deleteRecord = async () => {
    if (!confirm(`Delete “${event.title}”? Its itinerary, financial records, and documents will also be removed.`)) return;
    try {
...
function Overview({
  ...
  async function save() {
    if (!v.title.trim() || !v.starts_at) throw new Error('Title and start date are required.');
    const values = {
      ...v,
      title: v.title.trim(),
    ...
    if (creating) onCreated(await createEvent(values));else {
