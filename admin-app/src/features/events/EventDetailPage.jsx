...
export default function EventDetailPage() {
  ...
  return <div className="view active"><Link to={closeTo} aria-label="Close event workspace">← Events</Link><div className="view-header event-workspace-header"><h1 className="view-title">{creating ? 'New Event' : event.title}</h1>{!creating && <button className="btn-sm btn-sm-danger event-delete-button" onClick={async () => { if (!confirm(`Delete “${event.title}”? Its itinerary, financial records, and documents will also be removed.`)) return; try { await deleteEvent(id); navigate(closeTo); } catch (error) { alert(error.message); } }}>Delete Event</button>}</div><div className="tab-bar">{tabs.map((name) => <button key={name} className={`tab-btn ${tab === name ? 'active' : ''}`} onClick={() => setTab(name)}>{name}</button>)}</div>{needsSave ? <p className="empty-hint">Save the Overview first to set up this event workspace.</p> : <>{tab === 'Overview' && <Overview event={event} creating={creating} onSaved={reload} onCreated={(record) => navigate(`/admin/events/${record.id}`, { replace: true, state: location.state })} />}{tab === 'Itinerary' && <ItineraryManager eventId={id} />}{tab === 'Financials' && <>{['training', 'speaking'].includes(event.event_type) ? <p className="empty-hint">Billing is managed from the linked {event.event_type === 'training' ? 'Training' : 'Speaking Engagement'} workspace.</p> : <EngagementBillingPanel sourceType="event" sourceId={id} eventId={id} companyId={event.company_id} title={event.title} expectedOn={event.starts_at?.slice(0, 10)} initialValue={event.income_amount ?? ''} initialCertainty={event.status === 'confirmed' || event.status === 'completed' ? 'confirmed' : 'potential'} onValueSaved={async (amount) => { await updateEvent(id, { income_amount: amount }); await reload(); }} />}<ExpenseManager eventId={id} /></>}{tab === 'Documents' && <DocumentManager eventId={id} />}</>}</div>;
}
