...
export default function CalendarPage() {
  ...
  const reload = useCallback(async () => {
    ...
    setError(null);
    try {
      setData(await fetchCalendarRange(range.start.toISOString(), range.end.toISOString()));
    } catch (err) {
      setError(err);
    } finally {
      setLoading(false);
  ...
  return <div className="view active"><div className="view-header"><h1 className="view-title">Calendar</h1><div><button className="btn btn-ghost" aria-label="Previous month" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() - 1, 1))}>←</button><button className="btn btn-ghost" onClick={() => setMonth(new Date())}>Today</button><button className="btn btn-ghost" aria-label="Next month" onClick={() => setMonth(new Date(month.getFullYear(), month.getMonth() + 1, 1))}>→</button></div></div><h2 className="detail-section-title">{month.toLocaleString('en-US', { month: 'long', year: 'numeric' })}</h2>{error ? <section className="empty-hint" role="alert">Couldn’t load the calendar. <button className="btn-sm btn-sm-ghost" onClick={reload}>Try Again</button></section> : loading ? <p className="empty-hint">Loading calendar...</p> : <div className="calendar-grid">{names.map((name) => <div className="calendar-day-header" key={name}>{name}</div>)}{days.map((day) => <div className={`calendar-day-cell ${day.getMonth() !== month.getMonth() ? 'outside-month' : ''}`} key={key(day)}><div className="calendar-day-number">{day.getDate()}</div>{(events[key(day)] || []).map((event) => <button className={`calendar-event-chip event-type-${event.event_type}`} key={`${key(day)}-${event.id}`} onClick={() => open(event)}>{event.title}</button>)}{(tasks[key(day)] || []).map((task) => <button className={`calendar-task-chip ${task.status === 'done' ? 'task-done' : ''}`} key={task.id} onClick={() => navigate('/admin/tasks')}>{task.status === 'done' ? '✓ ' : '☐ '}{task.title}</button>)}</div>)}</div>}</div>;
}
