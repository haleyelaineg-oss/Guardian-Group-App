...
import { EXPENSE_CATEGORIES, EXPENSE_STATUSES, EXPENSE_TYPES, categoryForExpenseType, labelForExpenseStatus } from '../expenses/expenseOptions.js';
...
export default function DashboardPage() {
  ...
  return <div className="view active">{quickAction === 'task' && <QuickTaskModal eventOptions={data.eventOptions} onClose={() => setQuickAction(null)} onSaved={closeQuickAction} />}{quickAction === 'expense' && <QuickExpenseModal eventOptions={data.eventOptions} onClose={() => setQuickAction(null)} onSaved={closeQuickAction} />}{quickAction === 'mileage' && <QuickMileageModal eventOptions={data.eventOptions} onClose={() => setQuickAction(null)} onSaved={closeQuickAction} />}<div className="view-header"><h1 className="view-title">Dashboard</h1></div><section className="dashboard-quick-actions"><div className="detail-section-title">Quick Actions</div><div><button className="btn btn-primary" onClick={() => setQuickAction('task')}>+ Add Task</button><button className="btn btn-primary" onClick={() => setQuickAction('expense')}>+ Add Expense</button><button className="btn btn-primary" onClick={() => setQuickAction('mileage')}>+ Add Mileage</button></div></section><div className="stats-grid dashboard-operations-grid"><div className="stat-card accent"><div className="stat-value">{data.events.length}</div><div className="stat-label">Upcoming Events</div></div><div className="stat-card"><div className="stat-value">{data.tasks.length}</div><div className="stat-label">Open Tasks</div></div><div className="stat-card"><div className="stat-value">{data.speaking}</div><div className="stat-label">Active Speaking</div></div><div className="stat-card"><div className="stat-value">{data.trainings}</div><div className="stat-label">Active Trainings</div></div></div><div className="detail-section-title">Revenue Snapshot</div><div className="stats-grid"><RevenueCard value={revenue.booked} label="Total Revenue" sub="All Sources & Statuses YTD" /><RevenueCard value={revenue.earned} label="Earned Revenue" sub="Work completed, pending payment" /><RevenueCard value={revenue.accountsReceivable} label="Accounts Receivable" sub="open invoices" danger={Boolean(revenue.accountsReceivable)} /><RevenueCard value={revenue.collected} label="Collected Revenue" sub="received" accent /></div><div className="dashboard-split-row"><section><div className="dashboard-section-header"><div className="detail-section-title">Upcoming Events</div><Link className="dashboard-section-link" to="/admin/calendar">View calendar →</Link></div>{data.events.length ? data.events.map((event) => <button className="dashboard-event-row" type="button" key={event.id} onClick={() => openCalendarResource(event)}><span className="dashboard-event-main"><span className="dashboard-event-title">{event.title}</span><span className="dashboard-event-meta">{eventWhen(event)}{event.location ? ` · ${event.location}` : ''}</span></span><span className={`calendar-event-chip event-type-${event.event_type || 'other'}`}>{event.event_type || 'other'}</span></button>) : <p className="empty-hint">Nothing on the calendar yet.</p>}</section><section><div className="dashboard-section-header"><div className="detail-section-title">Open Tasks</div><Link className="dashboard-section-link" to="/admin/tasks">View tasks →</Link></div>{data.tasks.length ? data.tasks.map((task) => <div className="dashboard-event-row" key={task.id}><Link className="dashboard-event-main" to="/admin/tasks"><span className="dashboard-event-title">{task.title}</span><span className={`dashboard-event-meta ${task.due_date && task.due_date < today ? 'task-row-overdue' : ''}`}>{task.due_date ? formatDate(task.due_date) : 'No due date'}{task.owner && task.owner !== 'Unassigned' ? ` · ${task.owner}` : ''}</span></Link><input type="checkbox" aria-label={`Mark ${task.title} complete`} checked={task.status === 'done'} onChange={() => toggleTask(task)} /></div>) : <p className="empty-hint">No open tasks.</p>}</section></div></div>;
}
