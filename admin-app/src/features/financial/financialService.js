...
export async function fetchFinancialOverview() { const [documents, incomes, links, paymentAllocations, expenses] = await Promise.all([supabase.from('documents').select('id,doc_type,doc_number,client_name,status,total,balance,amount_paid,doc_date,due_date,date_paid').order('created_at', { ascending: false }), supabase.from('income').select('id,amount,certainty_status,income_kind,payment_path'), supabase.from('income_document_links').select('income_id,allocated_amount,documents(id,doc_type,status,due_date)'), supabase.from('payment_allocations').select('income_id,document_id,allocated_amount,payments(direction,received_at)'), supabase.from('expenses').select('amount,incurred_on')]); [documents, incomes, links, paymentAllocations, expenses].forEach((result) => fail(result.error)); return { documents: documents.data || [], incomes: incomes.data || [], links: links.data || [], paymentAllocations: paymentAllocations.data || [], expenses: expenses.data || [] }; }
export async function createGeneralExpense(values) { const { data, error } = await supabase.from('expenses').insert(values).select().single(); fail(error); return data; }
