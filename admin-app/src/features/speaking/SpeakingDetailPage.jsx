...
export default function SpeakingDetailPage() {
  ...
  const remove = async () => { if (!confirm(`Delete “${engagement.event_name}”? This will also remove it from the Calendar. Its planning records, documents, expenses, and care arrangements will be kept.`)) return; try { await deleteSpeakingEngagementAndCalendar(engagement); navigate('/admin/speaking'); } catch (error) { alert(error.message); } };
  return <div className="view active"><Link to="/admin/speaking" aria-label="Close speaking engagement workspace">← Speaking Engagements</Link><div className="view-header event-workspace-header"><h1 className="view-title">{engagement.event_name}</h1><button className="btn-sm btn-sm-danger event-delete-button" onClick={remove}>Delete Engagement</button></div><div className="tab-bar">{tabs.map((name) => <button key={name} className={`tab-btn ${tab === name ? 'active' : ''}`} onClick={() => setTab(name)}>{name}</button>)}</div>{tab === 'Overview' && <Overview engagement={engagement} onSaved={reload} />}{tab === 'Submissions' && <SpeakingSubmissions engagementId={engagement.id} submissions={data.submissions} onSaved={reload} />}{tab === 'Sessions' && <SpeakingSessions eventId={engagement.event_id} />}{tab === 'Prep' && <Checklist kind="speaking" engagementId={engagement.id} />}{tab === 'Travel' && <TravelPlanner eventId={engagement.event_id} startsAt={engagement.event_start_date} endsAt={engagement.event_end_date} />}{tab === 'Care' && <CareArrangements eventId={engagement.event_id} />}{tab === 'Financials' && <ExpenseManager eventId={engagement.event_id} incomeAmount={engagement.offered_fee} />}{tab === 'Documents' && <DocumentManager eventId={engagement.event_id} />}{tab === 'Post-Event' && <PostEvent engagement={engagement} onSaved={reload} />}</div>;
}

function Overview({ engagement, onSaved }) {
  const [v, setV] = useState(() => ({ event_name: engagement.event_name || '', organization_name: engagement.organization_name || '', event_website: engagement.event_website || '', venue: engagement.venue || '', venue_address: engagement.venue_address || '', city: engagement.city || '', region: engagement.region || '', country: engagement.country || '', zip_code: engagement.zip_code || '', event_start_date: engagement.event_start_date || '', event_end_date: engagement.event_end_date || '', cfp_deadline: engagement.cfp_deadline || '', application_url: engagement.application_url || '', contact_name: engagement.contact_name || '', contact_email: engagement.contact_email || '', contact_phone: engagement.contact_phone || '', requested_fee: engagement.requested_fee ?? '', offered_fee: engagement.offered_fee ?? '', status: normalizeSpeakingStatus(engagement.status), notes: engagement.notes || '' }));
