...
import { fetchPresentationSessions } from '../sessions/sessionService.js';
...
export default function SpeakingSessions({ eventId }) {
  ...
  useEffect(() => { fetchPresentationSessions().then((sessions) => setLibrarySessions(sessions.filter((session) => session.status !== 'archived'))).catch(() => {}); }, []);
  ...
  const openEditor = (row = null) => { setEditing(row?.id || 'new'); setValues(row ? formValues(row) : { ...BLANK, presentation_session_id: '' }); };
  const applyLibrarySession = (sessionId) => {
    const session = librarySessions.find((candidate) => candidate.id === sessionId);
    if (!session) { setValues((current) => ({ ...current, presentation_session_id: '' })); return; }
    setValues((current) => ({ ...current, presentation_session_id: session.id, title: session.title || '', session_type: session.session_type || '', speakers: Array.isArray(session.speakers) ? session.speakers.map((speaker) => typeof speaker === 'string' ? speaker : speaker.name).filter(Boolean).join(', ') : '', description: session.description || '', learning_objectives: session.learning_objectives || '', av_requirements: session.av_requirements || '' }));
  };
  ...
  async function save() {
    ...
    const payload = {
      ...
      presentation_session_id: values.presentation_session_id || null,
    };
