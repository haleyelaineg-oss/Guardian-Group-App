...
export default function SpeakingCreatePage() {
  ...
  const [addressBookContactId, setAddressBookContactId] = useState('');
  const [tab, setTab] = useState('Overview');
  const navigate = useNavigate();
  const location = useLocation();
  const closeTo = location.state?.returnTo || '/admin/speaking';
  const set = (key, value) => setV((old) => ({ ...old, [key]: value }));
  const chooseAddressBookContact = (contact) => {
    setAddressBookContactId(contact?.id || '');
    if (!contact) return;
    setV((old) => ({ ...old, contact_name: contact.full_name || '', contact_email: contact.email || '', contact_phone: contact.phone || '', organization_name: old.organization_name || contact.companies?.name || '' }));
  };
